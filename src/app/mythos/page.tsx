...
/* ─── SVG Constellation Particles (Hero background) ─── */
function ConstellationBg() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      circle.setAttribute("fill", Math.random() > 0.3 ? "#a855f7" : "#06b6d4");
      circle.setAttribute("opacity", `${0.2 + Math.random() * 0.5}`);
      svg.appendChild(circle);
      particles.push({ el: circle, x, y, vx: (Math.random() - 0.5) * 0.3, vy: (Math.random() - 0.5) * 0.3, r });
    }
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 150) {
            ...
            line.setAttribute("opacity", `${0.15 * (1 - dist / 150)}`);
  ...
  return (
    <svg ref={svgRef} viewBox="0 0 1400 900" className="my-constellation" preserveAspectRatio="xMidYMid slice" />
  );
}
...
function OrbitalRings() {
  return (
    <svg viewBox="0 0 600 600" className="my-orbital">
      ...
      <circle cx="300" cy="300" r="180" fill="none" stroke="#a855f7" strokeWidth="0.5" opacity="0.2" strokeDasharray="8 6">
        <animateTransform attributeName="transform" type="rotate" from="360 300 300" to="0 300 300" dur="45s" repeatCount="indefinite" />
      </circle>
      <circle cx="300" cy="300" r="120" fill="none" stroke="#06b6d4" strokeWidth="0.5" opacity="0.15" strokeDasharray="4 8">
  ...
  return (
    <div className="my-sep">
      <svg viewBox="0 0 800 40" preserveAspectRatio="none" className="my-sep-svg">
        ...
        <circle cx="400" cy="20" r="3" fill="#06b6d4" opacity="0.8">
...
function HierarchyDiagram() {
  ...
  useEffect(() => {
    ...
    const lines = svg.querySelectorAll<SVGLineElement>(".hier-line");
    lines.forEach((line) => {
    ...
    // Animate boxes
    animate(svg.querySelectorAll(".hier-box"), {
      opacity: [0, 1],
      scale: [0.8, 1],
      duration: 600,
      ease: "outBack(3)",
    ...
    // Animate text
    animate(svg.querySelectorAll(".hier-text"), {
      opacity: [0, 1],
      duration: 400,
    ...
    // Mythos glow pulse
    const glow = svg.querySelector(".hier-glow");
    if (glow) {
      animate(glow, {
        opacity: [0.2, 0.6, 0.2],
        r: [52, 58, 52],
  ...
  return (
    <div ref={ref}>
      <svg ref={svgRef} viewBox="0 0 720 120" className="my-hier-svg">
        <defs>
          <filter id="glow">
        ...
        {models.slice(0, -1).map((m, i) => (
          <line key={`line-${i}`} className="hier-line" x1={m.x + 55} y1={60} x2={models[i + 1].x - 15} y2={60} stroke="#06b6d4" strokeWidth="2" />
        ))}
        {models.map((m) => {
          const isMythos = m.name === "Mythos";
          return (
            <g key={m.name}>
              {isMythos && <circle className="hier-glow" cx={m.x + 20} cy={60} r={52} fill="#a855f7" opacity="0" />}
              <rect className="hier-box" x={m.x - 15} y={35} width={isMythos ? 80 : 70} height={50} rx={10} fill={isMythos ? "#1a0530" : "#110020"} stroke={isMythos ? "#a855f7" : "#2a1045"} strokeWidth={isMythos ? 2 : 1} filter={isMythos ? "url(#glow)" : undefined} opacity={0} />
              <text className="hier-text" x={m.x + (isMythos ? 25 : 20)} y={65} textAnchor="middle" fill={isMythos ? "#f0eaff" : "#a78bfa"} fontSize={isMythos ? 14 : 12} fontFamily="'DM Mono', monospace" fontWeight={isMythos ? 700 : 400} opacity={0}>{m.name}</text>
            </g>
          );
        })}
      </svg>
  ...
  return (
    <div ref={ref} className="my-gauge">
      <svg viewBox="0 0 80 80" width="64" height="64">
        <circle cx="40" cy="40" r="36" fill="none" stroke="#1a0530" strokeWidth="4" />
        <circle ref={circRef} cx="40" cy="40" r="36" fill="none" stroke="#a855f7" strokeWidth="4" strokeLinecap="round" strokeDasharray={circumference} strokeDashoffset={circumference} transform="rotate(-90 40 40)" style={{ filter: "drop-shadow(0 0 4px rgba(168,85,247,0.5))" }} />
        <text x="40" y="38" textAnchor="middle" fill="#f0eaff" fontSize="12" fontWeight="700" fontFamily="'DM Mono', monospace">{score}</text>
        <text x="40" y="52" textAnchor="middle" fill="#6b5b8a" fontSize="6" fontFamily="'DM Mono', monospace">{label}</text>
      </svg>
    </div>
  );
}
  ...
  return (
    <>
      <style>{`
        ...
        .mythos {
          --bg: #0a0014; --bg2: #1a0530; --surface: #110020; --border: #2a1045;
          --ink: #f0eaff; --ink2: #a78bfa; --ink3: #6b5b8a;
          --purple: #7c3aed; --purple-light: #a855f7; --purple-dark: #4c1d95; --cyan: #06b6d4;
        ...
        .my-orbital { position:absolute; width:600px; height:600px; top:50%; left:50%; transform:translate(-50%,-50%); pointer-events:none; z-index:0; opacity:0.6; }
        .my-nebula { position:absolute; border-radius:50%; filter:blur(50px); pointer-events:none; z-index:0; }
        .my-nebula-1 { width:600px; height:500px; top:-15%; left:-10%; background:radial-gradient(ellipse, rgba(139,92,246,0.18), transparent 70%); animation:float1 10s ease-in-out infinite; }
        .my-nebula-2 { width:700px; height:500px; bottom:-20%; right:-15%; background:radial-gradient(ellipse, rgba(168,85,247,0.12), transparent 70%); animation:float2 12s ease-in-out infinite; }
        .my-nebula-3 { width:400px; height:400px; top:15%; right:10%; background:radial-gradient(ellipse, rgba(6,182,212,0.08), transparent 70%); animation:float3 8s ease-in-out infinite; }
        .my-hero-content { position:relative; z-index:1; padding:0 2rem; }
        .my-hero-title { font-family:'Instrument Serif', serif; font-size:6rem; font-weight:400; letter-spacing:0.25em; text-shadow:0 0 100px rgba(168,85,247,0.6), 0 0 40px rgba(168,85,247,0.3); line-height:1; background:linear-gradient(135deg, #f0eaff 0%, #c084fc 50%, #f0eaff 100%); background-size:200% auto; -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; animation:shimmer 6s linear infinite; }
        .my-hero-sub { font-family:'DM Mono', monospace; font-size:1rem; color:var(--cyan); letter-spacing:0.5em; margin-top:1rem; text-transform:uppercase; }
        ...
        .my-label { font-family:'DM Mono', monospace; font-size:0.65rem; font-weight:500; letter-spacing:0.2em; text-transform:uppercase; color:var(--cyan); margin-bottom:0.75rem; }
        ...
        .my-quote { border-left:4px solid var(--purple); padding:1.25rem 1.5rem; background:rgba(124,58,237,0.06); border-radius:0 8px 8px 0; font-size:0.9rem; color:var(--ink); line-height:1.8; margin:1.5rem 0; }
        ...
        .my-accent-section { border-left:4px solid var(--cyan); padding-left:2rem; margin:2rem 0; }

        .my-hier-svg { width:100%; max-width:700px; display:block; margin:2.5rem auto; height:auto; }
        ...
        .my-bench-name { font-family:'DM Mono', monospace; font-size:0.7rem; letter-spacing:0.15em; text-transform:uppercase; color:var(--cyan); margin-bottom:0.3rem; }
        ...
        .my-bar-track { flex:1; background:rgba(26,5,48,0.8); border-radius:6px; height:28px; overflow:hidden; position:relative; }
        ...
        .my-bar-mythos { background:linear-gradient(90deg, var(--purple), var(--purple-light)); box-shadow:0 0 12px rgba(168,85,247,0.3); }
        ...
        .my-dramatic { font-family:'DM Mono', monospace; font-size:0.75rem; letter-spacing:0.15em; color:var(--purple-light); text-transform:uppercase; padding:0.5rem 1rem; background:rgba(168,85,247,0.08); border:1px solid rgba(168,85,247,0.15); border-radius:6px; display:inline-block; animation:pulse-glow 3s ease-in-out infinite; }
        ...
        .my-table td { padding:1rem 1.25rem; border-bottom:1px solid rgba(42,16,69,0.5); color:var(--ink2); }
        .my-table-highlight td { color:var(--ink); font-weight:600; background:rgba(168,85,247,0.05); box-shadow:inset 0 0 0 1px rgba(168,85,247,0.15); }
        ...
        .my-channel:hover { border-color:var(--purple); color:var(--ink2); }
        ...
        .my-source:hover { border-color:var(--purple); transform:translateY(-3px); box-shadow:0 8px 24px rgba(124,58,237,0.15); }
        ...
        <section className="my-section">
          ...
          <R d={80}>
            <div className="my-accent-section">
              ...
              <blockquote className="my-quote my-quote-cyan">「既知の脆弱性だけでなく、27年間発見されなかったOpenBSDのバグも自律的に発見した」</blockquote>
              <p className="my-text">この発見は、AIのセキュリティ研究能力が人間のエキスパートを補完し得ることを示す画期的な事例となった。</p>
            </div>
          </R>
        </section>
