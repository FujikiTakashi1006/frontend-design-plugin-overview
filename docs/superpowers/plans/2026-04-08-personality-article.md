# 「あなたの性格は、あなたが『選んだもの』じゃない」記事ページ Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a long-form science essay page at `/personality` with 7 sections, each featuring a unique SVG animation triggered by Intersection Observer, styled with a light-warm editorial design.

**Architecture:** Single `"use client"` page component at `src/app/personality/page.tsx` with inline `<style>` block. Uses a custom `useReveal` hook (same pattern as `src/app/best-practice/page.tsx`) to trigger CSS class additions on scroll. All SVG animations are CSS @keyframes driven — no JS animation libraries. After the page is complete, the homepage card is added.

**Tech Stack:** Next.js 16 App Router, React 19, CSS @keyframes, Intersection Observer API, Google Fonts (Playfair Display, Cormorant Garamond, Noto Sans JP)

**Spec:** `docs/superpowers/specs/2026-04-08-personality-article-design.md`

**Reference patterns:** `src/app/best-practice/page.tsx` (useReveal hook, R wrapper, SVG animation style)

---

## Chunk 1: Page Foundation — Styles, Hero, and Sections 01-02

### Task 1: Create page file with styles and useReveal hook

**Files:**
- Create: `src/app/personality/page.tsx`

- [ ] **Step 1: Create the page file with `"use client"`, Google Fonts import, full CSS `<style>` block, and `useReveal` hook**

Create `src/app/personality/page.tsx` with:

1. `"use client"` directive
2. React imports: `useEffect, useState, useRef`
3. `useReveal(threshold = 0.2)` hook — returns `{ ref, ok }`. Uses IntersectionObserver, disconnects after first intersection.
4. `R` reveal wrapper component — takes `ok` (boolean), `children`, `d` (delay in ms). Renders a `<div className="reveal">` that adds the `visible` class when `ok` is true. Uses opacity/translateY transition with `cubic-bezier(0.22,1,0.36,1)`, 0.85s duration, with `transition-delay` set to `d` ms.
5. Full `<style>` block with:
   - Google Fonts `@import` for Playfair Display (400, 400i), Cormorant Garamond (400, 600), Noto Sans JP (300, 400, 500)
   - CSS custom properties for the color palette:
     ```
     --bg: #f3efe8; --surface: #faf8f5; --border: #e0dbd4;
     --accent: #8b6f47; --accent-light: #c4a882; --muted: #a89a8b;
     --ink: #1a1a1a; --body: #3a3632; --sub: #6b6560; --error: #c4574a;
     ```
   - Base styles: body background, font-family, text color
   - `.hero` styles: min-height 100vh, flex center, text-align center, padding 60px 24px, relative, overflow hidden
   - `.hero::before` paper texture via data URI SVG: `background-image: url("data:image/svg+xml,%3Csvg width='60' height='60' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='60' height='60' filter='url(%23n)' opacity='0.03'/%3E%3C/svg%3E")`, position absolute inset 0, pointer-events none
   - `.hero-label`: Cormorant Garamond, 12px, letter-spacing 4px, uppercase, accent color
   - `.hero-title`: Playfair Display, clamp(28px, 5vw, 52px), weight 400, line-height 1.4, ink color
   - `.hero-title em`: italic, accent color
   - `.hero-subtitle`: Noto Sans JP, clamp(14px, 2vw, 18px), weight 300, sub color, max-width 540px
   - `.divider`: 60px wide, 1px height, accent-light background
   - `.hero-svg-container`: max-width 400px, margin auto
   - `.scroll-indicator`: absolute bottom 32px, flex column center
   - `.scroll-line`: 1px width, 40px height, gradient accent to transparent
   ...
   - `.section`: max-width 640px, margin auto, padding 80px 24px, border-top 1px solid var(--border)
   - `.section-number`: Cormorant Garamond, 13px, accent color, letter-spacing 3px
   - `.section-heading`: Playfair Display, clamp(20px, 3vw, 32px), weight 400, ink color, line-height 1.5
   - `.section-body`: Noto Sans JP, 15px, weight 300, line-height 2.0, body color
   - `.section-body p`: margin-bottom 1.5em
   - `.section-point`: background surface, border-left 3px solid accent, padding 20px 24px, border-radius 0 4px 4px 0
   - `.section-point p`: 14px, weight 500, ink color, line-height 1.8
   - Hero SVG keyframes:
     ```css
     @keyframes drawLine {
   ...
   - Section 02 SVG keyframes:
     ...
     ```
   - `.reveal` class: opacity 0, translateY 28px, transition 0.85s cubic-bezier(0.22,1,0.36,1)
   - `.reveal.visible`: opacity 1, translateY 0
   - `.svg-animate` class: marks SVG containers; child animations are paused by default
   - `.svg-animate.visible` child animations: running
   - Responsive: `@media (max-width: 640px)` — section padding 48px 16px, hero padding 40px 16px
6. Stub the default export `PersonalityPage` returning `<main>` with the `<style>` block rendered and a placeholder comment for content. The `<style>` must be included so fonts and variables are available from the start.

- [ ] **Step 2: Run dev server and verify page loads at /personality**

Run: `npm run dev` (if not already running), navigate to `http://localhost:3000/personality`
Expected: blank page loads without errors

- [ ] **Step 3: Commit**

```bash
git add src/app/personality/page.tsx
git commit -m "feat(personality): scaffold page with styles and useReveal hook"
```
...
**Files:**
- Modify: `src/app/personality/page.tsx`

- [ ] **Step 1: Add the Hero section JSX inside `<main>`**

**Note:** The Hero uses inline `style` animations (not `useReveal`/`R`) because it animates on page load, not on scroll. This is intentional — the Hero is always visible at the top.

Replace the placeholder with the full Hero section:
...
Add corresponding CSS classes for Hero SVG elements in the `<style>` block:
- `.connection`: stroke var(--accent-light), stroke-width 0.8, opacity 0, animation drawLine 2s ease forwards
- `.pulse`: fill none, stroke var(--accent), stroke-width 1, opacity 0, animation pulse 3s ease-in-out infinite
- `.node-appear`: opacity 0, animation nodeAppear 0.6s ease forwards

- [ ] **Step 2: Verify Hero renders correctly in browser**

Navigate to `http://localhost:3000/personality`
Expected: Hero section with animated title, SVG network, scroll indicator. Animations play on page load.

- [ ] **Step 3: Commit**

```bash
git add src/app/personality/page.tsx
git commit -m "feat(personality): add Hero section with SVG network animation"
```

---

### Task 3: Add Section 01 (導入) and Section 02 (社会脳)

**Files:**
- Modify: `src/app/personality/page.tsx`

- [ ] **Step 1: Add Section 01 below the Hero**

Section 01 has no SVG — text only. Use the `useReveal` hook and `R` wrapper for scroll reveal:
...
- [ ] **Step 2: Add Section 02 with SVG (社会脳 × ダンバー数)**
...
Add Section 02 SVG CSS classes to the `<style>` block:
- `.brain-outline`: stroke var(--accent-light), stroke-width 1.5, fill none
- `.brain-fill`: fill var(--accent), opacity 0; `.svg-animate.visible .brain-fill`: animation brainGrow 2s ease 0.5s forwards
- `.group-circle`: fill none, stroke var(--accent), stroke-width 1, stroke-dasharray 4 2, opacity 0
- `.svg-animate.visible .group-circle.g1/.g2/.g3`: animation ringAppear with 1s/1.5s/2s delays
- `.person-dot`: fill var(--accent), opacity 0
- `.svg-animate.visible .person-dot.d1` through `.d8`: animation dotIn with staggered delays (1.2s to 2.6s, 0.2s intervals)
- `.num-150`: Cormorant Garamond, 16px, fill var(--accent), opacity 0; `.svg-animate.visible .num-150`: animation fadeIn 1s ease 2.5s forwards
- `.svg-container`: max-width 400px, margin 32px auto

- [ ] **Step 3: Add Section01 and Section02 to the page's JSX (below Hero)**
...
- [ ] **Step 4: Verify in browser — scroll to see reveal animations and Section 02 SVG**

Navigate to `http://localhost:3000/personality`, scroll down
Expected: Section 01 text fades in on scroll. Section 02 heading and SVG appear — brain outline, rings expand, dots appear, "150" fades in.

- [ ] **Step 5: Commit**

```bash
git add src/app/personality/page.tsx
git commit -m "feat(personality): add Section 01 (intro) and Section 02 (social brain SVG)"
```

---

## Chunk 2: Sections 03-05

### Task 4: Add Section 03 (性格の正体 — if-then masks)

**Files:**
- Modify: `src/app/personality/page.tsx`

- [ ] **Step 1: Add Section 03 SVG CSS to the `<style>` block**
...
- [ ] **Step 2: Add Section03 component**
...
- [ ] **Step 3: Add `<Section03 />` to page JSX after Section02**

- [ ] **Step 4: Verify in browser — masks appear sequentially, arrows connect them, "if → then" label fades in**

- [ ] **Step 5: Commit**

```bash
git add src/app/personality/page.tsx
git commit -m "feat(personality): add Section 03 (if-then personality masks SVG)"
```

---

### Task 5: Add Section 04 (同調装置 — conformity waves)

**Files:**
- Modify: `src/app/personality/page.tsx`

- [ ] **Step 1: Add Section 04 SVG CSS to the `<style>` block**
...
- [ ] **Step 2: Add Section04 component**
...
- [ ] **Step 3: Add `<Section04 />` to page JSX**

- [ ] **Step 4: Verify in browser — waves appear, dots align, outlier pulses with ERROR**

- [ ] **Step 5: Commit**

```bash
git add src/app/personality/page.tsx
git commit -m "feat(personality): add Section 04 (conformity waves SVG)"
```

---

### Task 6: Add Section 05 (社会的伝染 — ripple contagion)

**Files:**
- Modify: `src/app/personality/page.tsx`

- [ ] **Step 1: Add Section 05 SVG CSS to the `<style>` block**
...
- [ ] **Step 2: Add Section05 component**
...
- [ ] **Step 3: Add `<Section05 />` to page JSX**

- [ ] **Step 4: Verify in browser — ripples pulse, nodes activate in layers, "57%" appears**

- [ ] **Step 5: Commit**

```bash
git add src/app/personality/page.tsx
git commit -m "feat(personality): add Section 05 (social contagion ripple SVG)"
```

---

## Chunk 3: Sections 06-07, Footer, and Homepage Integration

### Task 7: Add Section 06 (ソシオメーター — gauge)

**Files:**
- Modify: `src/app/personality/page.tsx`

- [ ] **Step 1: Add Section 06 SVG CSS to the `<style>` block**
...
- [ ] **Step 2: Add Section06 component**
...
- [ ] **Step 3: Add `<Section06 />` to page JSX**

- [ ] **Step 4: Verify in browser — gauge fills, needle swings with overshoot, env icons appear**

- [ ] **Step 5: Commit**

```bash
git add src/app/personality/page.tsx
git commit -m "feat(personality): add Section 06 (sociometer gauge SVG)"
```

---

### Task 8: Add Section 07 (結論 — network reconnect) and Footer

**Files:**
- Modify: `src/app/personality/page.tsx`

- [ ] **Step 1: Add Section 07 SVG CSS to the `<style>` block**
...
- [ ] **Step 2: Add Section07 component**
...
- [ ] **Step 3: Add Section07 and footer to page JSX**
...
- [ ] **Step 4: Verify full page in browser — scroll through all 7 sections, all SVGs animate correctly**

- [ ] **Step 5: Commit**

```bash
git add src/app/personality/page.tsx
git commit -m "feat(personality): add Section 07 (network reconnect SVG) and footer"
```

---

### Task 9: Add homepage card

**Files:**
- Modify: `src/app/page.tsx`

- [ ] **Step 1: Add the personality article to the `articles` array in `src/app/page.tsx`**

Insert at the beginning of the `articles` array (line ~27):
...
Take a screenshot of the completed `/personality` page hero section and save it as `/public/thumbs/personality.png` (1200x630px). Alternatively, create a simple SVG-based placeholder thumbnail with the page title and accent color (`#8b6f47`) on a `#f3efe8` background, saved as PNG.

- [ ] **Step 3: Verify homepage shows the new card**

Navigate to `http://localhost:3000`
Expected: New "Guide" card appears at the top of the article list with brown accent color. Clicking it navigates to `/personality`.

- [ ] **Step 4: Run linting to catch any errors**

Run: `npx eslint src/app/personality/page.tsx src/app/page.tsx`
Expected: No errors (warnings OK)

- [ ] **Step 5: Commit**

```bash
git add src/app/page.tsx public/thumbs/personality.png
git commit -m "feat: add personality article card to Knowledge Hub homepage"
```
...
**Files:**
- Review: `src/app/personality/page.tsx`

- [ ] **Step 1: Full page walkthrough in browser**

Navigate to `http://localhost:3000/personality` and scroll through the entire page. Check:
- Hero animations play on load
- Each section reveals on scroll (text + SVG)
- All 6 SVG animations trigger correctly
- Point boxes appear after text
- Typography is consistent (serif headings, sans-serif body)
- Colors match the palette
- Mobile responsive (resize browser to <640px)
- No console errors

- [ ] **Step 2: Run build to verify no TypeScript or compilation errors**

Run: `npm run build`
Expected: Build succeeds with no errors

- [ ] **Step 3: Commit any polish fixes if needed (skip if no changes)**

```bash
git add src/app/personality/page.tsx
git commit -m "polish(personality): final adjustments after visual review"
```
